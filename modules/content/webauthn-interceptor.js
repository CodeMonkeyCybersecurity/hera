// Hera WebAuthn Interceptor - MAIN World
// Intercepts navigator.credentials API calls and analyzes for security issues
// Sends findings to ISOLATED world via window.postMessage
...
(function() {
  ...
  if (typeof navigator === 'undefined' || !navigator.credentials || typeof navigator.credentials.create !== 'function') {
    console.log('Hera: WebAuthn interceptor not loaded - Credential Management API not available');
  ...
  if (typeof window !== 'undefined' && window.location &&
    ...
    console.log('Hera: WebAuthn interceptor not loaded - extension/internal page');
    return; // Exit early
  }
  ...
  /**
   * Send detection data to ISOLATED world via window.postMessage
   * TWO-SCRIPT ARCHITECTURE: MAIN world → postMessage → ISOLATED world → chrome.runtime
   */
  function sendToIsolatedWorld(subtype, data) {
    try {
      // SECURITY: Generate nonce to prevent replay attacks
      const nonce = crypto.randomUUID ? crypto.randomUUID() : Date.now().toString();

      // Send to ISOLATED world via postMessage
      // The relay script (webauthn-relay.js) will receive this and forward to background
      window.postMessage({
        source: 'hera-webauthn-interceptor',
        type: 'HERA_WEBAUTHN_DETECTION',
        subtype: subtype,
        nonce: nonce,
      ...
      // Silently fail - don't break WebAuthn
      const DEBUG = false;
      if (DEBUG) console.warn('Hera: Failed to send postMessage:', error);
    }
  }
  ...
  navigator.credentials.create = function(options) {
    ...
    if (issues.length > 0) {
      ...
      sendToIsolatedWorld('CREATE_ISSUES', {
        issues,
        options: {
    ...
    return originalCreate(options).then((credential) => {
      ...
      if (responseIssues.length > 0) {
        ...
        sendToIsolatedWorld('CREATE_RESPONSE_ISSUES', {
  ...
  navigator.credentials.get = function(options) {
    ...
    if (issues.length > 0) {
      ...
      sendToIsolatedWorld('GET_ISSUES', {
        issues,
        options: {
    ...
    return originalGet(options).then((assertion) => {
      ...
      if (responseIssues.length > 0) {
        ...
        sendToIsolatedWorld('GET_RESPONSE_ISSUES', {
          issues: responseIssues
        });
      }
